...
public class EditorStyleViewer : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        searchText = EditorGUI.DelayedTextField(new Rect(-16, 0, 102, 14), searchText, style: "toolbarSeachTextField");
        GUI.EndGroup();
        ...
        for (int i = 0; i < initialCount; i++)
        {
            ...
            if (CanShow(s.name))
            {
                ...
                {
                    stylePrefs[i].text = EditorGUI.DelayedTextField(new Rect(62, 20, 40, 14), stylePrefs[i].text);
